...
let answers = document.querySelector('.list-group-item');
...
function doAction(event){
    ...
    if (event.target.innerHtml === preQuestions[index].correct_answers){
        points ++;
...
function activateAnswers(){
    for (let i = 0; i < answers.length; i++){answers[i].addEventListener('click',doAction)
    }
}
activateAnswers();

function markCorrect(elem){
    elem.calssList.add('correct')
}